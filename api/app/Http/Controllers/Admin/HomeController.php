...
//            ->groupBy('users.id')
    ...
    {
        $nagad = SiteSettings::find(1);
        $nagad->nagad = $request->nagad;
        $nagad->binance = $request->binance;
        $nagad->bkash = $request->bkash;
        $nagad->update();
        return redirect()->back();
    }
    public function settings(){
        $nagad = SiteSettings::where('id',1)->first()->nagad;
        $binance = SiteSettings::where('id',1)->first()->binance;
        $bkash = SiteSettings::where('id',1)->first()->bkash;
        return Inertia::render('Profile/AccountSettings',['nagad'=>$nagad,'binance'=>$binance,'bkash'=>$bkash]);
    }
    ...
    public function userList(){
        $permissions = DB::table('admin_permissions')->paginate(10);
        $roles = DB::table('roles')->paginate(10);
        $admins = DB::table('subscriber')->where(['hold'=>0,'suspended'=>0])->paginate(10);
        $adminsDisabled = DB::table('subscriber')->where(['hold'=>1,'suspended'=>0])->paginate(10);
