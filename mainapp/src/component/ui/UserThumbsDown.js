...
export class UserThumb extends Component {
    ...
    render() {
        return (
               <View style={{position:'absolute',zIndex:10,bottom:80,width:250,height:40,backgroundColor:'orange',flexDirection:'row',justifyContent:'space-between'}}>
                <View style={{flexDirection:'row'}}>
                     <Image source={{uri:this.props.profilePicture}} style={{height:60,width:60,borderRadius:30,marginTop:-10,marginLeft:-15,borderWidth:3,borderColor:'orange'}} />
                <Text style={{fontSize:10,fontWeight:'bold',color:'white',letterSpacing:2,margin:5}}>{this.props.firstName.substring(0, 5)}</Text>
                </View>
