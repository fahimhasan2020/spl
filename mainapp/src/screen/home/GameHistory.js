  ...
  componentDidMount=()=>{
    ...
    render() {
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
          <Text style={{fontSize:19,color:COLORS.redDeep,fontWeight:'bold'}}>{item.game}</Text>
          <Text style={{fontSize:14,color:COLORS.redDeep}}>{item.result == 1 ?'Wins':'Lost'}</Text>
        </View>
        <View style={{flexDirection:'row',justifyContent:'space-between',marginTop:10}}>
          ...
          <Text style={{fontSize:13,color:'#cccccc'}}>{item.result == 1?'Earned:'+item.coins:'Lost:'+item.coins}</Text>
        </View>
      </View>}
