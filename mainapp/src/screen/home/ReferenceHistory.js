...
export class ReferenceHistory extends Component {
  ...
  componentDidMount=()=>{
    fetch(this.props.host+'user/rfhistory/'+this.props.userId).then((response)=>response.json()).then((responseJson)=>{
    ...
    render() {
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
          <Text style={{fontSize:19,color:COLORS.redDeep,fontWeight:'bold'}}>{item.user_name}</Text>
          <Text style={{fontSize:14,color:COLORS.redDeep}}>{item.game}</Text>
        </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(ReferenceHistory)
